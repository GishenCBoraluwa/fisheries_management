 ...
 */
function getOrCreateId(oldId, modelName) {
 ...
 */
function transformData(data, fileName, modelName) {
        ...
        try {
            ...
            fieldsToTransform.forEach(field => {
                if (transformed[field] && typeof transformed[field] === 'string') {
                    ...
                    let refModel = '';
                    if (field.includes('fishType') || field === 'fishTypeId')
                        refModel = 'fishTypes';
                    else if (field.includes('harbor') || field === 'harborId')
                        refModel = 'harbors';
                    else if (field.includes('driver') || field === 'driverId')
                        refModel = 'drivers';
                    else if (field.includes('truck') || field === 'truckId')
                        refModel = 'trucks';
                    else if (field.includes('user') || field === 'userId')
                        refModel = 'users';
                    else if (field.includes('order') || field === 'orderId')
                        refModel = 'orders';
                    else if (field.includes('route') || field === 'routeId')
                        refModel = 'deliveryRoutes';
                    else
                        refModel = modelName;
 ...
 */
async function seedModel(fileName) {
    const filePath = join(__dirname, "seedData", fileName);
    const modelKey = fileName;
    const prismaModelName = FILE_TO_MODEL_MAP[modelKey];
    const model = prisma[prismaModelName];
            ...
            try {
                ...
                if (record.id && typeof record.id === 'number') {
                    ...
                    if (oldIdField && jsonData[i] && typeof jsonData[i] === 'object' && jsonData[i] !== null) {
                        const originalId = jsonData[i][oldIdField];
 ...
 */
function getOldIdField(modelName) {
    const idFieldMap = {
        'fishTypes': 'fishTypeId',
        'harbors': 'harborId',
        'drivers': 'driverId',
